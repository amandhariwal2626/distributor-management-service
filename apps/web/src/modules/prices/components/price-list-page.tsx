  ...
  const columns = useMemo<ColumnDef<Price>[]>(
    () => [
      ...
      },
      {
        ...
        header: "MRP",
        cell: ({ row }) => (
          <span className="tabular-nums">₹{row.original.mrp.toFixed(2)}</span>
        ),
      },
      {
        id: "ptr",
        ...
        header: "PTR",
        cell: ({ row }) => (
          <span className="tabular-nums">₹{row.original.ptr.toFixed(2)}</span>
        ),
      },
      {
        id: "pts",
        ...
        header: "PTS",
        cell: ({ row }) => (
          <span className="tabular-nums">₹{row.original.pts.toFixed(2)}</span>
        ),
      },
      {
        id: "effectiveFrom",
        ...
        cell: ({ row }) => (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem
                onClick={() =>
                  router.push(`/prices/${row.original.id}/history`)
                }
              >
                View history
              </DropdownMenuItem>
        ...
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
          ...
          <Select
            ...
            onValueChange={(v) =>
              setFilters((f) => ({
                ...f,
                status: (v || undefined) as PriceStatus | undefined,
              }))
            }
          >
            <SelectTrigger>
