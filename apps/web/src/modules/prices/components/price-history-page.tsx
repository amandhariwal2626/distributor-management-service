...
export function PriceHistoryPage({ id }: { id: string }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <PageHeader
        title="Price History"
        description="Versioned timeline of changes for this price."
      />
      ...
      {isError ? (
        ...
        <EmptyState
          icon={History}
          title="No history"
          description="No revisions recorded for this price."
        />
      ) : (
                  ...
                  <div className="mt-3 space-y-1">
                    {v.changes!.map((c, i) => (
                      <div key={i} className="flex items-center gap-2 text-xs">
                        ...
                        <span className="rounded bg-red-100 px-1.5 py-0.5 font-mono text-red-600 line-through">
                          {String(c.oldValue)}
                        </span>
                        <span className="text-muted-foreground">→</span>
                        <span className="rounded bg-emerald-100 px-1.5 py-0.5 font-mono text-emerald-600">
                          {String(c.newValue)}
                        </span>
                      </div>
                    ))}
